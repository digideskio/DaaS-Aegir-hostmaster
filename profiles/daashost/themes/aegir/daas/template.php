...
function daas_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = daas_id_safe('page-' . $path);
    $vars['classes_array'][] = daas_id_safe('section-' . $section);
  ...
  // Add IE classes.
  if (theme_get_setting('daas_ie_enabled')) {
    $daas_ie_enabled_versions = theme_get_setting('daas_ie_enabled_versions');
    if (in_array('ie8', $daas_ie_enabled_versions, TRUE)) {
    ...
    if (in_array('ie9', $daas_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/css/ie9.css', array(
        'group' => CSS_THEME, 
        'browsers' => array('IE' => 'IE 9', '!IE' => FALSE), 
        'preprocess' => FALSE));
    }
    if (in_array('ie10', $daas_ie_enabled_versions, TRUE)) {
...
function daas_preprocess_page(&$vars, $hook) {
  ...
  if (in_array ( request_path (), array (
  		"",
		"hosting/sites",
		"hosting/platforms",
		"hosting/servers",
...
function daas_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from daas_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function daas_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function daas_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('daas_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {
    
    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('daas_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('daas_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('daas_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('daas_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *        
 */
function daas_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = daas_id_safe($element['#title']);
 ...
 */
function daas_preprocess_menu_local_task(&$variables) {
 ...
 */
function daas_menu_local_tasks(&$variables) {
  $output = '';
...
/**
 * Implements hook_preprocess_views_view Filters section
 *
 * Removing the fieldset from hosting_site_list view.
 */
function daas_preprocess_views_view(&$vars) {
  if ($vars['view']->name == 'hosting_site_list') {
    $vars['exposed'] = str_ireplace(array(
      '<fieldset class="collapsible collapsed form-wrapper">', 
      '</fieldset>'), '', $vars['exposed']);
  }
  return $vars;
}
 ...
 *
 * Removing the fieldset from hosting_site_list view Choose Operation list dropdown.
 */
function daas_form_alter(&$form, &$form_state, $form_id) {
  if (array_key_exists('views_bulk_operations', $form)) {
    if (($key = array_search('_hosting_site_collapse_views_fieldset', $form['#after_build'])) !== false) {
 ...
 * Adding a label before the operation select box.
 */
function daas_select($variables) {
  $output = '';
  if ($variables['element']['#id'] == 'edit-operation' && $variables['element']['#type'] == 'select') {
    $variables['element']['#options'][0] = ' ';
    $output = '<label> Choose Operation</label>' . theme_select($variables);
  }
  else {
    $output = theme_select($variables);
  }
  return $output;
}
